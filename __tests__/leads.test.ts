...
describe('LeadResolver', () => {
  ...
  describe('Fetch Single Lead', () => {
    it('should fetch a lead by ID', async () => {
      ...
      expect(response.body.data.lead.services[0]).toHaveProperty('id', 1);
      expect(response.body.data.lead.services[0]).toHaveProperty('name', 'delivery');
    });
